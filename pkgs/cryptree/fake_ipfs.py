...
IpfsAddtionResponse = TypedDict('AddtionResponse', {'Hash': str, 'Size': str})

class FakeIPFS:
    def __init__(self):
        self.datastore = {}

    def add_bytes(self, data: bytes, **kwargs) -> IpfsAddtionResponse:
        ...
        return {"Hash": cid, "Size": str(len(data))}
    
    def add_str(self, data: str, **kwargs) -> IpfsAddtionResponse:
        """Adds data to the fake IPFS and returns a CID (Content IDentifier)."""
        # Compute a simple CID using SHA256. This isn't the real algorithm IPFS uses, but it'll work for our mock version.
        cid = hashlib.sha256(data.encode()).hexdigest()
        self.datastore[cid] = data.encode()
        return {"Hash": cid, "Size": str(len(data.encode()))}
    
    def add_json(self, data: dict, **kwargs) -> IpfsAddtionResponse:
        """Adds data to the fake IPFS and returns a CID (Content IDentifier)."""
        # Compute a simple CID using SHA256. This isn't the real algorithm IPFS uses, but it'll work for our mock version.
        cid = hashlib.sha256(json.dumps(data).encode()).hexdigest()
        self.datastore[cid] = json.dumps(data).encode()
        return {"Hash": cid, "Size": str(len(json.dumps(data).encode()))}
