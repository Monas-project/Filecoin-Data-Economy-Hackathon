            ...
            <div className="bg-N92 h-full w-full flex flex-col text-N16 overflow-y-auto">
                <div className="w-full flex flex-col space-y-6 px-8 py-6 shadow-Elevation01 sticky top-0 bg-N92">
                    ...
                    <div className="flex flex-row justify-between">
                        ...
                        <div className="flex flex-row space-x-6">
                            <Button layout="neutral" headerVisible={true} headerIcon={<DocumentArrowUp20Regular />}>Upload File</Button>
                            <Button layout="neutral" headerVisible={true} headerIcon={<FolderAdd20Regular />}>Create Folder</Button>
                        </div>
                    </div>
                </div>
                <div className="w-full grow flex flex-col px-8 py-6 space-y-8">
                    <div className="w-full space-y-4">
                        ...
                        <div className="flex flex-row px-8 space-x-4">
                            <CompoundButton headerIcon={<FolderIcon />} layout="neutral" primaryText="AAAAAAAAAA" secondaryText="3 days ago" />
                        </div>
                    </div>
                                ...
                                <tr onClick={() => setIsSelected(!isSelected)}
                                    ...
                                    <td style={{ width: `${fileTableTr[3].width}%` }} className="space-x-5 pr-8 justify-end items-center">
                                        <div className={`space-x-3 flex-row group-hover:flex ${isSelected ? 'flex' : 'hidden'}`}>
                                            ...
                                            <Button onClick={() => setIsShareModalOpen(true)} layout="subtle" headerVisible={true} headerIcon={<Share20Regular />} labelVisible={false}></Button>
                                            <Button layout="subtle" headerVisible={true} headerIcon={<Delete20Regular />} labelVisible={false}></Button>
                                            <Button layout="subtle" headerVisible={true} headerIcon={<Key20Regular />} labelVisible={false}></Button>
                                        </div>
                                        <MoreVertical16Regular />
                                    </td>
                                </tr>
                ...
                {/* Share Button Dialog */}
                {isShareModalOpen && (
                    <div onClick={(e) => e.target === e.currentTarget && setIsShareModalOpen(false)}
                    ...
                    >
                        ...
                        <div className="relative top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/2 rounded-lg bg-N96 p-8 space-y-6">
                            ...
                            <Input id="address" layout="outlineLabel" size="larger" />
                            <div className="w-full flex flex-row justify-between">
                                <Button layout="neutral" size="large" onClick={() => setIsShareModalOpen(false)}>Close</Button>
                                <Button layout="primary" size="large" >send</Button>
                            </div>
                        </div>
